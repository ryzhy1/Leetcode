function arraySign(nums: number[]): any {
    // return ((nums.filter(x => x < 0).length) % 2 !=0) && (!(nums.includes(0))) ? -1 : ( (nums.reduce((a,b) => a*b)) != 0 ? 1 : 1);

    return ((nums.includes(0)) ? 0 : ((nums.filter(x => x < 0).length) % 2 !=0) ? -1 : 1);
}; 
